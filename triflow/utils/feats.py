...
import triflow.np.residue_constants as rc
...
def update_atom_pos(batch, tensor_6, outputs):

    device = outputs["sm"]["angles"].device
    float_dtype = outputs["sm"]["angles"].dtype
    ...
    #remove recycling dimension in batch
    new_batch = tensor_tree_map(lambda t: t[..., -1], batch)
    #convert all frames to global rigid frame
    rigid = Rigid.from_tensor_6(tensor_6)
    rigid = rigid.scale_translation(100)
    all_frames_to_global = torsion_angles_to_frames(rigid, outputs["sm"]["angles"][-1], new_batch["aatype"], default_frames)

    # get xyz coordinate in atom14 pos
    pred_xyz = frames_and_literature_positions_to_atom14_pos(all_frames_to_global, new_batch["aatype"], default_frames, group_idx, atom_mask, lit_positions)

    update_preds = {
        "frames": rigid.to_tensor_7(),
        "frames_6": tensor_6,
        "positions": pred_xyz
    }
    #update predictions
    outputs["sm"].update(update_preds)
    outputs["final_atom_positions"] = atom14_to_atom37(
        outputs["sm"]["positions"][-1], new_batch
    )

    #updating final_affine_tensor; final_atom_mask does not need to be updated here
